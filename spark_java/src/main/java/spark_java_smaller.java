...
public class spark_java_smaller {
    public static void main(String[] args) {
        SparkSession spark = SparkSession.builder().appName("spark_java_smaller").getOrCreate();
        Dataset<Row> smaller = spark.read().parquet("/anhlq36/spark/input/small.snappy.parquet"); //file Name
        ...
        //bai3
        countDF.repartition(1).write().mode("overwrite").format("parquet").save("/tiencm8/spark/smaller/device_model_num_user");
        lstUserIdDF.repartition(1).write().mode("overwrite").format("orc").save("/tiencm8/spark/smaller/device_model_list_user");
        //bai4
        ...
        action_by_button_id.repartition(1).write().mode("overwrite").format("parquet").save("/tiencm8/spark/smaller/button_count_by_user_id_device_model");
